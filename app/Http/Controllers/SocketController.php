...
class SocketController extends Controller implements MessageComponentInterface
{
    ...
    function onOpen(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['token'])) {
            User::where('token', $queryarray['token'])->update(['connection_id' => $conn->resourceId, 'user_status' => UserMessageStatus::ONLINE]);

            $user_id = User::select('id')->where('token', $queryarray['token'])->get();

            $data['id'] = $user_id[0]->id;

            $data['user_status'] = UserMessageStatus::ONLINE;
    ...
    function onClose(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['token'])) {
            User::where('token', $queryarray['token'])->update(['connection_id' => 0, 'user_status' => UserMessageStatus::OFFLINE]);

            $user_id = User::select('id', 'updated_at')->where('token', $queryarray['token'])->get();

            $data['id'] = $user_id[0]->id;

            $data['user_status'] = UserMessageStatus::OFFLINE;
